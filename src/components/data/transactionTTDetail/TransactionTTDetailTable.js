...
const Component = ({
    ...
    const filteredList = testServiceTypeList.filter((record) => {
        // Kiểm tra nếu `tdlServiceName` và `tdlServiceCode` không phải null
        const hasValidName = record.tdlServiceName && record.tdlServiceName.toLowerCase().includes(searchTerm.toLowerCase());
        const hasValidCode = record.tdlServiceCode && record.tdlServiceCode.toLowerCase().includes(searchTerm.toLowerCase());
        
        // Lọc nếu một trong hai điều kiện đúng
