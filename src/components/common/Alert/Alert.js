...
const Alert = ({ message, type = "success", onClose }) => {
    ...
    const icon =
        type === "success" ? <FaCheckCircle className="mr-2 h-6 w-6" /> : <FaTimesCircle className="mr-2 h-6 w-6" />;

    const bgColor = type === "success" ? "bg-green-600" : "bg-red-600";

    return (
        <div className={`flex items-center p-3 text-white ${bgColor} rounded shadow-md text-lg 
